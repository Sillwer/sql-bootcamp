...
--
-- Name: mv_dmitriy_visits_and_eats; Type: MATERIALIZED VIEW; Schema: public; Owner: postgres
--

CREATE MATERIALIZED VIEW public.mv_dmitriy_visits_and_eats AS
 WITH visited AS (
         SELECT pizzeria.name AS pizzeria_name
           FROM ((( SELECT person_visits.id,
                    person_visits.person_id,
                    person_visits.pizzeria_id,
                    person_visits.visit_date
                   FROM public.person_visits
                  WHERE (person_visits.visit_date = '2022-01-08'::date)) visit
             JOIN public.person ON ((person.id = visit.person_id)))
             JOIN public.pizzeria ON ((pizzeria.id = visit.pizzeria_id)))
        ), with_good_price AS (
         SELECT pizzeria.name AS pizzeria_name
           FROM (( SELECT menu.id,
                    menu.pizzeria_id,
                    ...
                    menu.price
                   FROM public.menu
                  WHERE (menu.price < (800)::numeric)) m
             JOIN public.pizzeria ON ((pizzeria.id = m.pizzeria_id)))
        )
 SELECT visited.pizzeria_name
   FROM visited
INTERSECT
 SELECT with_good_price.pizzeria_name
   FROM with_good_price
  WITH NO DATA;


ALTER MATERIALIZED VIEW public.mv_dmitriy_visits_and_eats OWNER TO postgres;
...
--
-- Name: mv_dmitriy_visits_and_eats; Type: MATERIALIZED VIEW DATA; Schema: public; Owner: postgres
--
